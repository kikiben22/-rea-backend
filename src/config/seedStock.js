/**
 * Seed stock médicaments REA
 * node src/config/seedStock.js
 */
...
async function seedStock() {
  ...
  console.log('\n🏥 Stock REA initialisé avec succès!');
}

/* Version appelée par server.js (connexion déjà active) */
async function seedStockQuiet() {
  await insertMeds();
}

async function insertMeds() {
  const existing = await Medicament.countDocuments();
  if (existing > 0) {
    console.log(`⚠️  Stock déjà peuplé (${existing} médicaments) — suppression et rechargement...`);
    await Medicament.deleteMany({});
  }

  const now = new Date();
  const docs = STOCK.map(m => ({
    ...m,
    dateExpiration: new Date(now.getFullYear() + 2, now.getMonth(), 1),
    numerLot: `LOT-${Date.now()}-${Math.floor(Math.random()*1000)}`,
    mouvements: [{
      type: 'ENTREE',
  ...
  console.log(`💊 ${docs.length} médicaments ajoutés dans icu_monitoring`);
  docs.filter(d => d.quantiteStock <= d.seuilAlerte).forEach(d =>
    console.log(`   ⚠️  Alerte stock: ${d.nom} (${d.quantiteStock} ${d.unite})`)
  );
}

module.exports = { seedStockQuiet };

if (require.main === module) {
  seedStock().catch(err => { console.error(err); process.exit(1); });
}
